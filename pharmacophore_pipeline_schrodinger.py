...
Parallelism (all local, no remote job server / token consumption):
    - LigPrep:          -NJOBS 24 -HOST localhost -LOCAL -NOJOBID
    - ConfGen:          -NJOBS 24 -HOST localhost -LOCAL -NOJOBID
    - Phase find_common: -HOST localhost:24 -LOCAL -NOJOBID
...
def prepare_ligands(input_mae: str,
    ...
    cmd = [
        ...
        "-LOCAL",                # keep files in current dir (no token checkout)
        "-WAIT",                 # block until complete
        "-NOJOBID",              # no job ID registration
    ]
...
def generate_conformers(input_mae: str,
    ...
    cmd = [
        ...
        "-NJOBS", str(NCPUS),    # split across all cores
        "-HOST",  "localhost",   # run locally (no remote job server)
        "-LOCAL",                # keep files in current dir
        "-WAIT",
        "-NOJOBID",
    ]
...
def build_pharmacophore(actives_mae: str,
    ...
    if not os.path.isfile(phzip):
        # Try looking for it in the same directory
        phzip_alt = os.path.join(output_dir, f"{jobname}.phzip")
        if os.path.isfile(phzip_alt):
            phzip = phzip_alt
        else:
            raise FileNotFoundError(f"Expected .phzip not found at {phzip}")
    ...
    hypo_subdir = os.path.join(fc_dir, "hypotheses")
    if not os.path.isdir(hypo_subdir):
        hypo_subdir = fc_dir  # fallback: hypotheses at root level
    ...
    scores_csv = os.path.join(fc_dir, "scores.csv")
    best_hypo = None
    best_score = -1.0
