...
\begin{frame}{Series E: Model Evaluation on Test Compounds}
  \centering
  \small
  \begin{tabular}{lccc}
    \toprule
  ...
  \begin{columns}
    ...
    \centering
    \begin{tikzpicture}[scale=0.75]
      % Scatter plot: IC50 measured vs predicted (log scale)
      % Range: 6.07 - 108 µM (log: 0.783 to 2.033)
      \draw[->] (0,0) -- (5.3,0) node[right]{\tiny log(Measured IC$_{50}$)};
      \draw[->] (0,0) -- (0,5.3) node[above]{\tiny log(Pred.\ IC$_{50}$)};
      % Ticks at 10, 30, 100 µM
      \foreach \x/\lbl in {0.74/10, 2.35/30, 3.96/100} {
        \draw (\x,0.06) -- (\x,-0.06) node[below]{\tiny\lbl};
        \draw (0.06,\x) -- (-0.06,\x) node[left]{\tiny\lbl};
      }
      \draw[red,dashed,thick] (0,0) -- (5,5);
      \foreach \p in {0.74, 2.35, 3.96} {
        \draw[gray!30] (\p,0) -- (\p,5);
        \draw[gray!30] (0,\p) -- (5,\p);
      }
      % Data points: (log(meas), log(pred))
      \fill[steelblue] (1.250,1.117) circle(3pt);
      \fill[steelblue] (1.804,1.533) circle(3pt);
      \fill[steelblue] (0.783,0.868) circle(3pt);
      \fill[steelblue] (1.086,1.047) circle(3pt);
      \fill[steelblue] (2.033,1.436) circle(3pt);
      \fill[steelblue] (1.230,1.233) circle(3pt);
    \end{tikzpicture}
    \column{0.48\textwidth}
    \begin{alertblock}{Performance Metrics}
      \large
      \begin{itemize}
        ...
        \item \textbf{Max error}: 3.95$\times$ (ROY-0000275)
        \item \textbf{Median error}: 1.23$\times$
      \end{itemize}
    \end{alertblock}
    \vspace{0.2em}
    {\small One compound ($\times$3.95 error) shows underprediction; others within 1.4$\times$.}
  \end{columns}
\end{frame}
