...
\begin{frame}{Series C: Model Evaluation on Test Compounds}
  ...
  \begin{tabular}{lccc}
    \toprule
    \textbf{Compound ID} & \textbf{Measured pIC$_{50}$} & \textbf{Predicted pIC$_{50}$} & \textbf{Residual} \\
    \midrule
    ROY-0000173-001 & 5.328 & 5.302 & $-$0.026 \\
    ROY-0000174-001 & 5.009 & 5.183 & $+$0.174 \\
    ROY-0000220-001 & 6.107 & 5.931 & $-$0.176 \\
    ROY-0000221-001 & --- (>200\,$\mu$M) & 4.848 & --- \\
    ROY-0000175-001 & 5.165 & 5.215 & $+$0.050 \\
    ROY-0000211-001 & 5.745 & 5.909 & $+$0.164 \\
    \midrule
  ...
  \begin{columns}
    ...
    \begin{tikzpicture}[scale=0.75]
      % Axes
      \draw[->] (0,0) -- (5.2,0) node[right]{\tiny Measured pIC$_{50}$};
      \draw[->] (0,0) -- (0,5.2) node[above]{\tiny Predicted pIC$_{50}$};
      % Tick labels (scaled: 5.0->0, 5.5->2.5, 6.0->5)
      \foreach \x/\lbl in {0/5.0, 2.5/5.5, 5/6.0} {
      ...
      % grid
      \foreach \p in {2.5,5} {
        \draw[gray!40,dashed] (\p,0) -- (\p,5);
        \draw[gray!40,dashed] (0,\p) -- (5,\p);
      }
      % Data points: (meas,pred) scaled as (val-5.0)*10
      % (5.328,5.302) -> (3.28,3.02)
      % (5.009,5.183) -> (0.09,1.83)
      % (6.107,5.931) -> (11.07,9.31) ... use *5/2
      % scale = (val-5.0)/(6.2-5.0)*5 => (val-5.0)*4.167
      \foreach \mx/\py in {3.27/3.01, 0.38/7.63, 4.46/3.88, 0.69//0.00, 3.10/3.81}\fill[steelblue] (\mx,\py) circle(3pt);
    \end{tikzpicture}
    \column{0.48\textwidth}
    ...
    \end{alertblock}
    \vspace{0.3em}
    {\small Predictions within $\approx\! 0.12$ pIC$_{50}$ on average --- equivalent to $\sim\!32\%$ error in IC$_{50}$ --- acceptable for virtual screening.}
  \end{columns}
\end{frame}
...
\begin{frame}{Series E: Prospective Predictions (No Measured Data)}
  ...
  \begin{tabular}{lcccl}
    \toprule
    \textbf{Compound ID} & \textbf{Pred.\ pIC$_{50}$} & \textbf{Pred.\ IC$_{50}$ ($\mu$M)} & \textbf{Cluster} & \textbf{Note} \\
    \midrule
    ROY-0000308 & 4.883 & 13.08 & 1  & Fluorophenyl \\
    ROY-0000307 & 4.467 & 34.11 & 1  & Bromopyridine (weakest) \\
    ROY-0000287 & 5.132 & 7.38  & $-$1 & Hexyloxy (best predicted) \\
    ROY-0000286 & 4.953 & 11.15 & $-$1 & Phenylpropyloxy \\
    ROY-0000275 & 4.563 & 27.33 & 1  & Bromopyridine \\
    ROY-0000268 & 4.767 & 17.09 & 1  & Alkyne-phenyl \\
    \midrule
    \multicolumn{2}{l}{\textbf{Range}} & 7.38 -- 34.11\,$\mu$M & \multicolumn{2}{l}{pIC$_{50}$: 4.47 -- 5.13} \\
    \bottomrule
  \end{tabular}
  \vspace{0.5em}
  ...
  \begin{columns}
    ...
    \begin{block}{Structural Features}
      \small
      \begin{itemize}
        \item All contain spiro-piperidine (Boc-protected) core
        \item ROY-0000287 (hexyloxy chain) has best predicted potency
        \item Halopyridine compounds predicted as weakest
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{alertblock}{Prediction Uncertainty}
      \small
      \begin{itemize}
        \item No measured IC$_{50}$ available for Series E
        \item Ensemble uncertainty = 0 (uniform predictions --- extrapolation region)
        \item Experimental validation needed
      \end{itemize}
    \end{alertblock}
  \end{columns}
\end{frame}
